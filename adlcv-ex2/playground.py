...
def image_to_patches(image, patch_size, image_grid=True):
    ...
    if image_grid:
        ...
        patches = rearrange(image, 'b c (nph ph) (npw pw) -> b c nph ph npw pw', ph=patch_h, pw=patch_w)
        print(f'shape of patches1:  {patches.shape}')
        ...
        """
        patches = rearrange(patches, 'b c nph ph npw pw -> b (nph npw) c ph pw')

        print(f'shape of patches2:  {patches.shape}')
        ...
        print(f'shape of patches3:  {patches.shape}')
        patches = rearrange(patches, 'b c nph ph npw pw -> b (nph npw) (c ph pw)')
