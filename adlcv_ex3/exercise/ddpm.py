...
class Diffusion:
    def __init__(self, T=500, beta_start=1e-4, beta_end=0.02, img_size=16, device="cuda", schedule='linear'):
        """
        ...
        self.betas = self.get_betas(schedule).to(device)
    ...
    def get_betas(self, schedule='linear'):
        ...
        # add your own (e.g. cosine)
            ...
            cosine_schedule = (torch.cos(torch.linspace(-math.pi, 0, self.T)) + 1) / 2  # This will give values between 0 and 1
            # Adjust the cosine schedule to be between beta_start and beta_end
            return self.beta_start + (self.beta_end - self.beta_start) * cosine_schedule
        else :
    ...
    def p_mean_std(self, model, x_t, t):
        ...
        #print(alpha.shape, alpha_bar.shape, beta.shape)
    ...
    def p_sample_loop(self, model, batch_size, timesteps_to_save=None):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(reversed(range(1, self.T)), position=0, total=self.T-1):
                ...
                x = self.p_sample(model, x, t).to(self.device)
