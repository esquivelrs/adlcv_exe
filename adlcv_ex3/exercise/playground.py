...
if __name__ == '__main__':
    ...
    diffusion = Diffusion(device=device, schedule='linear')
    diffusion_cos = Diffusion(device=device, schedule='cosine')

    plt.figure()
    plt.plot(range(1,diffusion.T+1), diffusion.betas.cpu().numpy(), label='betas_linear', linewidth=3)
    plt.plot(range(1,diffusion_cos.T+1), diffusion_cos.betas.cpu().numpy(), label='betas_cosine', linewidth=3)
    plt.xlabel('time steps')
    plt.title('Diffusion parameters')
    plt.legend()
    plt.savefig('assets/diffusion_params_beta.png', bbox_inches='tight')
    plt.show()
    #####################################################
    ...
    # TASK 1: Implement beta, alpha, and alpha_hat 
    plt.figure()
    plt.plot(range(1,diffusion.T+1), diffusion.alphas.cpu().numpy(), label='alphas_linear', linewidth=3)
    plt.plot(range(1,diffusion.T+1), diffusion.alphas_bar.cpu().numpy(), label='alphas_bar_linear',linewidth=3)
    plt.plot(range(1,diffusion.T+1), diffusion.betas.cpu().numpy(), label='betas_linear', linewidth=3)
    plt.plot(range(1,diffusion_cos.T+1), diffusion_cos.alphas.cpu().numpy(), label='alphas_cosine', linewidth=3)
    plt.plot(range(1,diffusion_cos.T+1), diffusion_cos.alphas_bar.cpu().numpy(), label='alphas_bar_cosine',linewidth=3)
    plt.plot(range(1,diffusion_cos.T+1), diffusion_cos.betas.cpu().numpy(), label='betas_cosine', linewidth=3)
    plt.title('Diffusion parameters')
    plt.xlabel('time steps')
    plt.legend()
    ...
    t = torch.Tensor([0, 50, 100, 150, 200, 250, 300, 350, 400, 450, 499]).long().to(device)
    ...
    xt_cos, noise_cos = diffusion_cos.q_sample(x0, t)
    #####################################################

    noised_images = np.stack([im_normalize(tens2image(xt[idx].cpu())) for idx in range(t.shape[0])], axis=0)
    noised_images_cos = np.stack([im_normalize(tens2image(xt_cos[idx].cpu())) for idx in range(t.shape[0])], axis=0)
    show(noised_images, title='Forward process (linear)', fig_titles=fig_titles, save_path='assets/forward.png')
    show(noised_images_cos, title='Forward process (cosine)', fig_titles=fig_titles, save_path='assets/forward_cosine.png')
