...
class VGG(nn.Module):
    def __init__(self, num_classes=5):
        ...
        vgg_pretrained_features = torchvision.models.vgg11(pretrained=True).features
        self.features = nn.Sequential(*list(vgg_pretrained_features)[:10])
...
def frechet_distance(mu1, sigma1, mu2, sigma2):
    ...
    diff = mu1 - mu2    
    cov_prod = np.dot(sigma1, sigma2)


    fid = np.dot(diff, diff) + np.trace(sigma1 + sigma2 - 2*linalg.sqrtm(cov_prod))
