...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        keys = rearrange(keys, 'b s (h d) -> b s h d', h=self.num_heads)
        ...
        queries = rearrange(queries, 'b s (h d) -> b s h d', h=self.num_heads)
        queries = rearrange(queries, 'b s h d -> (b h) s d', h=self.num_heads)

        values = rearrange(values, 'b s (h d) -> b s h d', h=self.num_heads)
